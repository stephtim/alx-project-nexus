...
## Render (example)
- Create a service, set build to use `ecommerce_backend/Dockerfile`
- Provide environment variables
- Use worker for celery commands
 - Jenkins: add credentials `DB_NAME`, `DB_USER`, `DB_PASS`, `SECRET_KEY` (type: Secret text)
	 The provided `Jenkinsfile` will write a runtime `ecommerce_backend/.env` and run `docker-compose up --build -d`.
